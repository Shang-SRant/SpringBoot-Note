...
Spring Boot其设计目的是用来`简化Spring应用的 初始搭建以及开发过程`。该框架使用了`特定的方式来进行配置`，从而使开发人员不 再需要定义样板化的配置。通过这种方式，Spring Boot致力于在蓬勃发展的快速应用开发领域成为领导者。
...
```markdown
# 1.使用 @Repository @Service @Controller @Component管理不同简单对象
...
```markdown
http://localhost:8080/cmfz/index.jsp
```
...
bir:	<span th:text="${user.bir}"></span>
		<span th:text="${#dates.format(user.bir, 'yyyy-MM-dd HH:mm')}"></span> 日期格式化
```

##### 10.3.3 条件展示数据

```java
 model.addAttribute("user",new User("21","zhangsan",20,new Date()));
```
...
spring:
  ...
  datasource:
    ...
    driver-class-name: com.mysql.cj.jdbc.Driver		#指定驱动(连接时需指定时区)
    url: jdbc:mysql://localhost:3306/springboot		#指定url
    username: root									#指定用户名
...
mybatis:
  mapper-locations: classpath:com/stq/mapper/*.xml  #指定mapper配置文件位置
  type-aliases-package: com.stq.entity              #指定起别名来的类
```
...
<mapper namespace="com.stq.dao.ClazzDAO">
...
# 1.开启自动编译
	...
	File -> Setting -> Build, Execution, Deployment -> Compiler -> 勾选上 Build project automatically 这个选项

# 2.开启允许在运行过程中修改文件
	ctrl + alt + shift + / -> 选择Registry -> 勾选 compiler.automake.allow.when.app.running 这个选项
```
...
2019-07-17 21:23:18.782  INFO 4496 --- [  restartedMain] s.b.c.e.t.TomcatEmbeddedServletContainer : Tomcat initialized with port(s): 9999 (http)
2019-07-17 21:23:18.796  INFO 4496 --- [  restartedMain] o.apache.catalina.core.StandardService   : Starting service [Tomcat]
2019-07-17 21:23:18.797  INFO 4496 --- [  restartedMain] org.apache.catalina.core.StandardEngine  : Starting Servlet Engine: Apache Tomcat/9.0.30
```
...
<configuration>
    ...
    <!--项目中指定包日志控制-->
    <logger name="com.stq.dao" level="DEBUG"/>
...
public class MyAspect {
    @Before("execution(* com.stq.service.*.*(..))")
    public void before(JoinPoint joinPoint){      //joinPoint为切入点
        System.out.println("前置通知");
        
        //均可通过joinPoint获取切入点信息
...
@Component
public class MyAspect {
    @After("execution(* com.stq.service.*.*(..))")
    public void before(JoinPoint joinPoint){      //joinPoint为切入点
...
@Component
public class MyAspect {
    @Around("execution(* com.stq.service.*.*(..))")
    public Object before(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {      //proceedingJoinPoint为切入点
        System.out.println("进入环绕通知");
        
        //均可通过proceedingJoinPoint获取切入点信息
